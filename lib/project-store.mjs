...
import { parseBuildingCanonical, parseScaffoldConfig } from "./schemas/validate-canonical.mjs";
...
/** @param {string} id */
export async function readScaffoldConfig(id) {
  return JSON.parse(await readFile(join(dataDir(id), "scaffold-config.json"), "utf8"));
}

/** @param {string} id @param {Record<string, unknown>} data */
export async function writeMeta(id, data) {
  const now = new Date().toISOString();
  const meta = { ...data, id, updated_at: now };
  await writeFile(join(dataDir(id), "meta.json"), JSON.stringify(meta, null, 2));
  return meta;
}

/** @param {string} id @param {unknown} data */
export async function writeCanonical(id, data) {
  const parsed = parseBuildingCanonical(data);
  await writeFile(
    join(dataDir(id), "building-canonical.json"),
...
/** @param {string} id @param {unknown} data */
export async function writeScaffoldConfig(id, data) {
  const parsed = parseScaffoldConfig(data);
  await writeFile(
    join(dataDir(id), "scaffold-config.json"),
    JSON.stringify(parsed, null, 2),
  );
  const meta = await readMeta(id);
  await writeMeta(id, meta);
